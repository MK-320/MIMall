...
> **‼️⁉️ 重要说明**：本仓库主要是前端项目。由于原始后端服务不可用，项目内置了 `koa-mock/` 作为本地 Mock API 服务，用于模拟接口与登录态，**不等同于真实后端**。如果你有可用的后端服务，可以修改 `vue.config.js` 中的代理配置或 `src/main.js` 中的 `axios.defaults.baseURL` 来接入真实后端。
...
本仓库包含 `LICENSE` [MIT](https://github.com/MK-320/MIMall/blob/main/LICENSE) 文件，使用前请先阅读并遵守相应许可协议。
